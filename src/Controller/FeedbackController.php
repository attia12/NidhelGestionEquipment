...
class FeedbackController extends AbstractController
{
    ...
    #[Route('/approve/{id}', name: 'app_feedback_approve', methods: ['GET','POST'])]
    public function approve(Feedback $feedback, EntityManagerInterface $entityManager,EmailService $emailService): Response
{
    $feedback->setApproved(true);
    $entityManager->flush();
    $emailService->sendApprovalEmail($feedback->getEmail(), $feedback->getName());

    return $this->redirectToRoute('app_feedback_index');
}
#[Route('/disapprove/{id}', name: 'app_feedback_disapprove', methods: ['GET','POST'])]
public function disapprove(Feedback $feedback, EntityManagerInterface $entityManager): Response
{
